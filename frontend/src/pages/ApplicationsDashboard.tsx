...
import { Typography, Box, IconButton, Pagination, Paper, Select, MenuItem, FormControl, InputLabel, TextField, InputAdornment, Button } from '@mui/material';
...
export const ApplicationsDashboard = ({ jobs, setJobs, setEditingJob, setIsModalOpen, setShowRejection, isMobile }: ApplicationsDashboardProps) => {
    ...
    const paginatedJobs = sortedJobs.slice((page - 1) * rowsPerPage, page * rowsPerPage);
                    ...
                    <Box display="flex" alignItems="center" gap={1}>
                        ...
                        >
                            {[9, 30, 50, 100, 200].map(size => (
                                <MenuItem key={size} value={size} sx={{ fontSize: '0.75rem' }}>{size}</MenuItem>
                            ))}
                        </Select>
                    </Box>
                </Box>
