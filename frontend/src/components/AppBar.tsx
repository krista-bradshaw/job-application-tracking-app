import { AppBar as MuiAppBar, Toolbar, Box, Typography, Button, IconButton } from "@mui/material";
...
export const AppBar = ({ theme, isMobile, isAiEnabled, activeTab, setActiveTab, setIsSettingsOpen, colorMode, logout }: AppBarProps) => (
    <MuiAppBar
        position="sticky"
        elevation={0}
        color="inherit"
    ...
    >
        <Toolbar sx={{ display: 'flex', justifyContent: 'space-between', px: { xs: 1, sm: 2 } }}>
            ...
            <Box display="flex" alignItems="center" sx={{ gap: { xs: 1, sm: 2 } }}>
                ...
                {/* Tabs */}
                <Box sx={{ display: 'flex', backgroundColor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider', alignItems: 'flex-start' }}>
                    <Button
            ...
            <Box display="flex" alignItems="center" >
                {isAiEnabled && (
                    <IconButton onClick={() => setIsSettingsOpen(true)} color="inherit" size={"medium"}>
                        <SettingsIcon fontSize={"medium"} />
                    </IconButton>
                )}
                <IconButton onClick={colorMode.toggleColorMode} color="inherit" size={"medium"}>
                    {theme.palette.mode === 'dark' ? <LightModeIcon fontSize={"medium"} /> : <DarkModeIcon fontSize={"medium"} />}
                </IconButton>
                <IconButton onClick={logout} color="inherit" title="Logout" size={"medium"}>
